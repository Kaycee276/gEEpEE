...
export const CognitiveTerminal: React.FC<CognitiveTerminalProps> = ({ rebalanceResult, isRebalancing }) => {
  return (
    <div className="neo-card" style={{ padding: '16px', background: '#ffffff' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.72rem', color: '#555555', fontWeight: 700, marginTop: '2px' }}>
            Data: Step-by-step AI cognitive reasoning log, Litmus Gate checks, and onchain Base transaction verification.
          </p>
        </div>
      ...
      {isRebalancing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <p style={{ fontSize: '0.8rem', fontWeight: 900, color: '#000000' }}>Executing Memory Recall & Base DEX Swap...</p>
          ...
          {rebalanceResult.reasoning_steps?.map((step, idx) => (
            <div key={idx} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                ...
                <span style={{ fontSize: '0.85rem', fontWeight: 900, color: '#000000' }}>{step.title}</span>
              </div>
              <p style={{ fontSize: '0.8rem', color: '#222222', marginTop: '4px', fontWeight: 700 }}>{step.detail}</p>
            </div>
          ))}

          {rebalanceResult.executed_swap && (
            <div style={{ background: 'var(--neo-green-light)', border: '2px solid #000000', boxShadow: '3px 3px 0px #000000', padding: '10px 12px' }}>
              <div style={{ color: '#000000', fontWeight: 900, fontSize: '0.88rem', marginBottom: '4px', textTransform: 'uppercase' }}>
                Onchain Base Swap Verified
              </div>
              <p className="font-mono" style={{ fontSize: '0.78rem', color: '#000000', fontWeight: 700, wordBreak: 'break-all' }}>
